  ...
  <div class="finder-home">
    ...
    <div class="relative mt-1 rounded-md shadow-sm">
      ...
      <div class=" absolute inset-y-0 right-0 flex items-center pr-3">
        <svg v-if="emptyStringOnSubmit" class="h-5 w-5 text-red-500" xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          <path fill-rule="evenodd"
            d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-5a.75.75 0 01.75.75v4.5a.75.75 0 01-1.5 0v-4.5A.75.75 0 0110 5zm0 10a1 1 0 100-2 1 1 0 000 2z"
            clip-rule="evenodd" />
        </svg>
        <svg @click="submitForm" v-if="!emptyStringOnSubmit" width="24" height="24"
          class="text-indigo-500 cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd"
          clip-rule="evenodd">
          <path fill="indigo"
    ...
    <p v-if="emptyStringOnSubmit" class="text-left mt-2 sm:text-sm text-red-600 lg:text-base" id="empty-string-error">
      The book title must have atleast one character</p>
    <pulse-loader class="mt-12" :loading="isLoading" :color="'#A5B4FC'"></pulse-loader>
  </div>
        ...
        submitForm(){
          ...
          if(this.bookName){
            ...
            const bookTitleSplit = this.bookName.split(' ').join('+');
              this.store.setBooksData(bookTitleSplit).then(() => {
